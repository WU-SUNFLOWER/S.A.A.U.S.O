...
FrameObject* FrameObject::NewInstance(Handle<PyFunction> func,
  ...
  // 遍历kwarg_keys
  for (auto i = 0; i < actual_kw_arg_cnt; ++i) {
    auto key =
  ...
  Handle<PyTuple> extend_pos_args;
  if (code_object->flags() & PyCodeObject::Flag::kVarArgs) {
    extend_pos_args = PyTuple::NewInstance(actual_extend_pos_arg_cnt);
  }
  ...
  if (actual_extend_pos_arg_cnt > 0) {
    ...
    auto extend_pos_args_idx = 0;
    for (auto i = 0; i < actual_extend_pos_arg_cnt; ++i) {
      extend_pos_args->SetInternal(extend_pos_args_idx,
                                   actual_args->Get(formal_pos_arg_cnt + i));
      ++extend_pos_args_idx;
    }
  }
