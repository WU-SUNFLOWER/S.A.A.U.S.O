...
Maybe<void> Context::Set(Local<String> key, Local<Value> value) {
  ...
  i::Handle<i::PyDict> globals =
      i::handle(i::Tagged<i::PyDict>::cast(*context_object), i_isolate);
  ...
  auto maybe_set =
      i::PyDict::Put(
          globals, i::handle(i::Tagged<i::PyObject>::cast(*py_key), i_isolate),
          py_value, i_isolate);
...
MaybeLocal<Value> Context::Get(Local<String> key) {
  ...
  auto maybe_found = globals->Get(
      i::handle(i::Tagged<i::PyObject>::cast(*py_key), i_isolate), out,
      i_isolate);
