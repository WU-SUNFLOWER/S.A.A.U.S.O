...
- **代码审查建议顺序**：
  1. 对外头文件（`include/saauso-embedder*.h`）
  2. 桥接层（`src/api/`）
  3. 运行时调用链（`src/runtime/runtime-py-function.cc`、`src/objects/py-function-klass.cc`）
  4. 示例与测试（`examples/embedder/`、`test/embedder/`）
...
- 错误与异常门面：读 [exception-state.h](file:///e:/MyProject/S.A.A.U.S.O/src/execution/exception-state.h)、[exception-utils.h](file:///e:/MyProject/S.A.A.U.S.O/src/execution/exception-utils.h) 与 [runtime-exceptions.h](file:///e:/MyProject/S.A.A.U.S.O/src/runtime/runtime-exceptions.h)（pending exception + Runtime\_Throw\* + 便捷宏）。
...
- **objects (`src/objects/`)**：对象表示层（布局/字段/GC iterate/instance\_size）+ vtable slot 的调度骨架；`py-xxx.cc` 实体类优先保持为“数据容器 + 原语操作”，避免把跨对象的复杂语义流程塞进实体类。
...
- 若被调用对象是 type object：走 [PyTypeObjectKlass::Virtual\_Call](file:///e:/MyProject/S.A.A.U.S.O/src/objects/py-type-object-klass.cc#L176-L181)。
...
- 在实现中处理参数的通用约定：
  ...
  - 尽量复用 runtime helper：例如 iterable 展开使用 [Runtime\_UnpackIterableObjectToTuple](file:///e:/MyProject/S.A.A.U.S.O/src/runtime/runtime-iterable.cc#L43-L64)，list 扩展使用 `Runtime_ExtendListByItratableObject`，避免在类型实现中重复写迭代逻辑。
...
- **测试目录分层（按模块分类）**：
  - `test/unittests/interpreter/`：解释器相关用例，主要为 `interpreter-*-unittest.cc`，也包含专项文件（如 `native-print-unittest.cc`）。
    - 该目录内部较为复杂。更多细节，务必参考文档 `test/unittests/interpreter/README.md`
