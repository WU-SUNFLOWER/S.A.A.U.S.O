...
- `Runtime_NewObject` 作为统一门面：先调用 `own_klass->NewInstance(args, kwargs)`，再调用 `own_klass->InitInstance(instance, args, kwargs)`，见 [runtime-reflection.cc](file:///e:/MyProject/S.A.A.U.S.O/src/runtime/runtime-reflection.cc#L169-L179)。

**默认实现的语义（何时会被用到）**

- `Klass::InitializeVTable()` 会把 `new_instance/init_instance` 初始化为默认逻辑，见 [klass-default-vtable.cc](file:///e:/MyProject/S.A.A.U.S.O/src/objects/klass-default-vtable.cc#L42-L66)。
- 默认 `new` 负责分配“通用 PyObject”；默认 `init` 负责查找并调用 `__init__`（若存在），适用于普通 Python 类，不适用于需要特定内存布局的内建容器/数值类型。
...
- `Isolate::InitMetaArea()` 会先对每个 `Klass` 执行 `InitializeVTable()`，再执行 `PreInitialize()`（见 [isolate.cc](file:///e:/MyProject/S.A.A.U.S.O/src/execution/isolate.cc)），因此：
  - `PreInitialize()` 是覆盖 `vtable_.new_instance` / `vtable_.init_instance` 的正确地点；
  - 如果忘记覆盖，类型会回退到默认 `new/init` 逻辑。

**推荐实现模板（遵循仓库习惯，强调可测性）**

- 在 `*-klass.h` 中声明：
  - `static MaybeHandle<PyObject> Virtual_NewInstance(Tagged<Klass> klass_self, Handle<PyObject> args, Handle<PyObject> kwargs);`
  - `static Maybe<void> Virtual_InitInstance(Tagged<Klass> klass_self, Handle<PyObject> instance, Handle<PyObject> args, Handle<PyObject> kwargs);`
- 在 `PreInitialize()` 中接入：
