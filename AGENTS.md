...
- **允许直接使用 PyDict 原语的场景**（当你实现的是解释器更高层语义时）：
  ...
  - 这类场景必须使用 fallible API：`Get/GetTagged/PutMaybe/RemoveMaybe/ContainsMaybe`，并在失败时立即传播 pending exception。
- **内部初始化路径的约束**：
  - 即便是 key 可证明为 interned `PyString`（例如 `ST(...)` / 字节码 names 表），也必须使用 `PutMaybe/Get/GetTagged/...`。
