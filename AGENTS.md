...
### 1.2. Embedder API（外部嵌入接口）能力概览

Embedder API 用于将 S.A.A.U.S.O 作为脚本引擎嵌入到宿主 C++ 程序中，主要能力包括：

- 创建并管理 `Isolate/Context`，并以 RAII 方式进入/退出作用域。
- 编译与执行脚本（`Script::Compile/Run`），并用 `TryCatch` 捕获错误。
- 在宿主与脚本间交换基础值与对象（`String/Integer/Float/Boolean/Object/List/Tuple` 等）。
- 向脚本侧注入宿主回调（例如 `Function::New`），并通过互调链路与 VM 侧实现衔接。
...
特别地，**当“风格规范”与“能编译/能跑单测”的事实冲突时，以仓库现状与可验证性为最高优先级（见 [Saauso-AI-Developer-Contributing-Guide.md](file:///e:/MyProject/S.A.A.U.S.O/docs/Saauso-AI-Developer-Contributing-Guide.md)）。**
