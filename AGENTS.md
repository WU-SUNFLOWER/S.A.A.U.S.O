...
- **已具备**：
  ...
  - 最小异常对象体系：`Isolate::ExceptionState` + `Runtime_Throw*` 抛错门面，并在 `builtins` 中注入 `BaseException/Exception/TypeError/RuntimeError/ValueError/IndexError/KeyError/NameError/AttributeError/ZeroDivisionError/StopIteration` 等最小异常类型对象（用于错误格式化与后续异常传播的演进）。
...
- **builtins 字典（行为对齐用）**：
  ...
  - 同时会注入最小异常类型对象体系：`BaseException/Exception` 以及 `TypeError/RuntimeError/ValueError/IndexError/KeyError/NameError/AttributeError/ZeroDivisionError/StopIteration` 等。
