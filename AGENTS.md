...
- **旧 dict API 已废弃**：
  - `PyDict::Get/GetTagged/Put/Contains/Remove` 无法表达三态（命中/未命中/异常），在 key 可能触发 `__hash__/__eq__` 的路径上容易造成异常覆盖/吞掉。
  - 新代码默认不得引入旧 API；代码审查/AI 生成代码需要遵循本条。
...
- **唯一允许保留旧 API 的场景**：
  - key 可证明为 interned `PyString`（例如 `ST(...)` / 字节码 names 表），且调用点没有异常传播通道（例如初始化/void 路径）；该场景也应尽量在代码注释中说明“仅限 PyString key”。
