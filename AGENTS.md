...
-  - `Runtime_NewObject` 作为统一门面：先调用 `own_klass->NewInstance(args, kwargs)`，仅当 `__new__` 返回值是“当前被调用类型（或其子类）的实例”时才继续调用 `own_klass->InitInstance(instance, args, kwargs)`（对齐 `type.__call__` 语义），见 [runtime-reflection.cc](file:///e:/MyProject/S.A.A.U.S.O/src/runtime/runtime-reflection.cc#L236-L279)。
...
-  - 默认 `getattr` 流程（`PyObjectKlass::Generic_GetAttr`）：先尝试 `AccessorProxy`（当前覆盖 `__class__` / `__dict__` 特殊属性），再查实例 dict（仅 heap object），再沿 MRO 查类字典，最后再尝试 `__getattr__`（见 [py-object-klass.cc](file:///e:/MyProject/S.A.A.U.S.O/src/objects/py-object-klass.cc#L98-L190) 与 [accessor-proxy.cc](file:///e:/MyProject/S.A.A.U.S.O/src/builtins/accessor-proxy.cc)）。
...
-  - 除 `print/repr/len/isinstance/__build_class__/sysgc/exec` 外，还会注入 `object/int/str/float/list/bool/dict/tuple/type` 的 type 对象、`True/False/None` 单例，并把 `builtins` 自身注册进 builtins dict（自引用）。
...
-  - builtins 的 C++ 实现在 `src/builtins/`，其函数签名为 `MaybeHandle<PyObject> Builtin_Xxx(Handle<PyObject> receiver, Handle<PyTuple> args, Handle<PyDict> kwargs)`。
...
-  - 当前解释器主路径已接入 pending-exception 传播与 exception table unwind；仍保留少量 fail-fast 路径（例如未知 opcode 时 `stderr + exit(1)`）。
...
-  - 当前语义：`ModuleNotFoundError` 仅用于模块未命中（loader 返回 `None`）场景；模块体执行过程中抛出的异常会按 pending-exception 原样传播。
- **内建模块（builtin）**：
