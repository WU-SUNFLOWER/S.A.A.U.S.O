...
- **容器 `cast` 语义约定（重要）**：
  ...
  - 判定口径优先使用 layout kind（例如 `IsListLike`），而不是仅用 `IsPyList` 这类 exact klass 判定。
  ...
  - 若某调用点确实需要“exact 类型语义”（例如做语言层行为分歧、调试诊断或兼容性守卫），应显式使用 `IsPyXxx`/exact 检查，不要把 exact 语义隐式塞回 `T::cast`。
- **对象布局关键约束**:
