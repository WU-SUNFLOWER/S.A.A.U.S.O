...
- `BUILD.gn`：根目标定义（核心 `saauso_core`、可选 `saauso_cpython312_compiler`、公共 `embedder_api`、CLI `vm`、Embedder 示例、统一测试入口 `ut`）。
...
- `ut`：统一测试入口（聚合 `//test/unittests:ut` 与 `//test/embedder:embedder_ut`）。
...
- **新增测试文件的接入点**：
  ...
  - 根 `BUILD.gn` 仅保留 `group("ut")` 作为统一入口，避免测试 sources 污染根构建文件。
