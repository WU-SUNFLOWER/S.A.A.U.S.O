...
- `Runtime_NewObject` 作为统一门面：先调用 `own_klass->NewInstance(args, kwargs)`，再调用 `own_klass->InitInstance(instance, args, kwargs)`，见 [runtime-reflection.cc](file:///e:/MyProject/S.A.A.U.S.O/src/runtime/runtime-reflection.cc#L216-L241)。
...
- **MRO/C3 与属性查找**:
  ...
  - 默认 `getattr` 流程（`PyObjectKlass::Generic_GetAttr`）：先查实例 dict（仅 heap object），再沿 MRO 查类字典，最后再尝试 `__getattr__`（见 [py-object-klass.cc](file:///e:/MyProject/S.A.A.U.S.O/src/objects/py-object-klass.cc#L92-L175)）。
