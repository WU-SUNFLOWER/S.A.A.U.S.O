...
- **已具备**：
  ...
  - 字节码解释器：基于 CPython 3.12 字节码模型的执行引擎（computed-goto dispatch）、栈帧与参数绑定；解释器侧 `builtins` 通过 `BuiltinBootstrapper` 统一构建与注入（基础 builtins、基础类型名、单例与最小异常类型等）。
  ...
  - 异常处理（解释器侧）：引入 CPython 3.11+ 的 `co_exceptiontable`（exception table）解析与 handler 查找能力，用于后续逐步补齐更真实的异常展开语义。
...
- `src/builtins/`：内建函数与“内建类型方法集合”的实现层；内建函数统一使用 `Builtin_*` 函数签名；内建类型方法集合通常由各 `PyXxxKlass::Initialize()` 安装到 type dict（例如 `str/list/dict/tuple/type` 及其 iterator/view 的 methods）。
...
- **理由**：
  ...
  - 控制职责边界：Isolate 负责状态（如 ExceptionState），Execution 负责门面（Call/builtins），interpreter 负责算法（dispatch/unwind）。

#### 3.0.2. Error Conventions（必须遵守）

- **Fallible API（可能抛 Python 异常）**：
  - 返回类型必须为 `MaybeHandle<T>`
  - 失败时必须返回“空 MaybeHandle”（`kNullMaybeHandle`），并且必须已设置 `Isolate::exception_state()` 的 pending exception
  - 调用方必须显式检查 `IsEmpty()/ToHandle(...)` 并继续向上返回失败或进入 unwind
- **Try/Lookup API（不会抛异常，仅返回缺失）**：
  - 返回类型保持为 `Handle<T>` / `Tagged<T>`
  - 返回 `Handle<T>::null()` 仅表示缺失（not found），并且必须保证不设置 pending exception
- **执行入口**：
...
- **旧 dict API 已移除（禁止再引入）**：
  - 历史上的 `PyDict::Get/GetTagged/Put/Contains/Remove` 已从仓库中删除，原因是它们无法表达三态（命中/未命中/异常），容易在 key 触发 `__hash__/__eq__` 时造成异常覆盖/吞掉。
  - 若你在分支/旧提交中看到这些 API：需要迁移到本节列出的新约定；不要把旧 API 重新加回仓库。
...
- **允许直接使用 PyDict 原语的场景**（当你实现的是解释器更高层语义时）：
  ...
  - 这类场景必须使用 fallible API：`GetTaggedMaybe/PutMaybe/RemoveMaybe/ContainsMaybe`，并在失败时立即传播 pending exception。
- **内部初始化路径的约束**：
  - 即便是 key 可证明为 interned `PyString`（例如 `ST(...)` / 字节码 names 表），也必须使用 `PutMaybe/GetTaggedMaybe/...`。
...
- **StringTable**: 常用字符串常量池（通常在 `kMetaSpace` 分配字符串对象，避免被 GC 移动）。
  - **文件组织**：按领域拆分为 `runtime-*.{h,cc}`（truthiness/iterable/intrinsics/conversions/reflection/exec/string），各 header 只暴露本领域的对外入口，避免出现“万能头文件”导致的编译期耦合。
  - **推荐放置规则**：
...
- **builtins 字典（行为对齐用）**：
  - builtins 字典由 `BuiltinBootstrapper` 统一构建（避免 Interpreter 构造函数膨胀），见 [builtin-bootstrapper.h](file:///e:/MyProject/S.A.A.U.S.O/src/interpreter/builtin-bootstrapper.h)。
  - 除 `print/len/isinstance/__build_class__/sysgc/exec` 外，还会注入 `object/int/str/float/list/bool/dict/tuple/type` 的 type 对象、`True/False/None` 单例，并把 `builtins` 自身注册进 builtins dict（自引用）。
...
- **关键语义点（以仓库现状为准）**：
  ...
  - 已知问题：当模块体执行过程中抛出异常时，当前实现可能会把该失败路径误报为 `ModuleNotFoundError`（TODO：应保留并传播真实 pending exception）。
- **内建模块（builtin）**：
...
### 4.0. AI 自检清单（提交前必过）
- **目标**：避免“上帝函数”和“无注释代码”，让人类 Reviewer 能快速判断正确性与可维护性。
- **函数边界**
...
- **尽量让函数短小且职责单一**
  ...
  - 然而，如果一个函数**超过了约 40 行**，必须评估是否可以在不破坏程序结构的前提下，拆分为**更小、更易于调试和维护或更易复用的片段**。
  - 拆分优先级建议：
