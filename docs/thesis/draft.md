...
在完成虚拟机堆、句柄机制和对象系统之后，S.A.A.U.S.O VM 才具备“让 Python 程序运行起来”的基础。但推动字节码指令逐条执行的解释器并不是执行层的全部；在此之前，首先需要完成运行时入口、内建能力、模块子系统以及代码装载等基础设施的开发。换言之，本节解决的并不是“如何逐条执行字节码指令”，而是“在解释器真正启动之前，必须先把哪些基础设施前提搭好”。字节码解释器则放到下一节集中讨论。
