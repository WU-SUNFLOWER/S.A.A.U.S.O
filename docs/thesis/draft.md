...
正如 2.6 中所讨论的，虽然模块导入在运行时由解释器通过导入相关字节码触发，但其后续语义并不适合直接堆叠在解释器内部完成，而应交由专门的模块子系统继续处理。也正因如此，S.A.A.U.S.O VM 将模块系统设计为执行层中的相对独立子系统，而不是把名称解析、模块定位、模块加载、缓存维护等逻辑零散地耦合进解释器主循环。这样的划分既有助于控制解释器复杂度，也更便于后续扩展相对导入、包结构和模块缓存等能力。
