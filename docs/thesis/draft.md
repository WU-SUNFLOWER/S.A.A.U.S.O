...
FrameObject* FrameObjectBuilder::BuildSlowPath(...) {
  auto code_object = func->func_code(isolate);
  ...
  return FrameObject::Create(
      code, func, bound_locals, func->func_globals(), 
...
Handle<PyObject> Interpreter::CallPythonImpl(Handle<PyObject> callable,
  ...
  if (IsNormalPyFunction(callable)) {
    FrameObject* frame =
        FrameObjectBuilder::Build(isolate_, callable, 
...
这段代码中 `POP_JUMP_IF_FALSE` 指令的实现也直接回应了 2.2 的理论分析：跳转指令通过修改程序计数器改变控制流，而条件判断则依赖运行时语义子系统提供的对象真值语义。
