...
如 2.3.4 所述，一个 PVM 即使已经拥有对象系统和解释器，如果没有基本的内建运行环境，仍然无法真正“可用”。因此，S.A.A.U.S.O VM 在执行层中专门组织了内建能力子系统，用于承载内建能力的具体实现。在现阶段的系统中，`Isolate` 容器会持有一个 `builtins` 字典。全体内建函数的 C++ 实现，会在 `Isolate` 容器初始化时被包装成 Python 函数对象并注入进 `builtins` 字典。在后文 4.5.3 中将会进一步说明，当用户 Python 代码需要调用内建函数时，解释器最终就会访问这个字典并凭目标内建函数的名称进行查询。
