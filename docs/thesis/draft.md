...
到此为止，我们已经基本明确 Python 中函数对象的动态创建机制，以及需要这种动态机制的原因。这对本文后续的 PVM 系统设计有直接影响。S.A.A.U.S.O VM 在后续实现中不应当把“函数”简单视为一段代码，而是将其建模为一种特殊对象：既包含代码对象等静态的元数据，又持有运行时阶段产生的上下文信息。至于函数对象为何还需要绑定其定义时所属的全局命名空间，以及这与后文栈帧中的 `globals` 字段有何关系，将在下一小节结合函数调用进一步说明。

### 2.3.2 栈帧与函数调用的原理

Python 中的函数调用通常是通过 `CALL` 等字节码发起的。
...
对于栈帧中的“局部环境”本身，还需要作进一步区分。站在 Python 语义层面，PVM 至少要能够表达当前函数可见的局部命名空间，以及该函数按词法作用域绑定的全局命名空间；但站在解释器实现层面，若仍让所有局部变量都通过字典按名字查找，则执行代价会过高。因此，一个实际可用的 PVM 往往需要同时维护两层结构：一层是与 Python 语义直接对应的命名空间映射，另一层则是为提高局部变量访问效率而引入的内部槽位区。

在现代 CPython 3.12 的执行模型中，对于单个 Python 文件的顶层代码，也就是可视作“根函数”的那部分代码，PVM 往往直接在字典形式的命名空间中完成名称读写，因此编译器更可能生成 `LOAD_NAME`、`STORE_NAME` 等按名字解析的字节码。此时，当前栈帧中的 `locals` 与 `globals` 在逻辑上都指向该模块的命名空间，它们可以重合。相对地，对于一般的 Python 函数，形参与普通局部变量通常会被预先编排进一个按槽位顺序访问的局部变量区；在后文中，本文将沿用项目代码中的术语，把它称为 `localsplus` 数组。解释器随后通过 `LOAD_FAST`、`STORE_FAST` 直接按下标访问这些槽位，而不再在每次读写时都进行字典查找。

`localsplus` 的意义在于，它把形参、普通局部变量、Cell 变量和 Free 变量预先编排进一个定长槽位区，使解释器能够通过 `LOAD_FAST`、`STORE_FAST`、`LOAD_DEREF`、`STORE_DEREF` 等字节码直接按槽位下标访问对应值，而不需要像通过全局变量那样每次都执行按名字查找。这样一来，PVM 既保留了 Python 语义上“名称绑定”的外在表现，又能在函数热点路径上获得更高的局部变量访问效率。

与此同时，函数体对模块级全局变量的访问又是另一条路径。若函数内部需要读取其所在模块中的全局名称，编译器通常会生成 `LOAD_GLOBAL`，解释器在运行时会先查该函数绑定的全局命名空间，再查内建命名空间；若函数体显式使用 `global` 声明并对某个名称赋值，则相应写操作会通过 `STORE_GLOBAL` 落到该函数绑定的全局命名空间之中。至于闭包场景，将在后文 2.3.4 中单独予以讨论。

这一点可以通过一个简单例子更直观地理解。假设有两个模块 `mod_a.py` 和 `mod_b.py`：
```python
# mod_a.py
x = 10
def func():
...
其中 `func` 函数读取变量 `x` 的操作是通过 `LOAD_GLOBAL` 字节码指令完成的。当解释器执行 `mod_a.py` 的顶层代码时，会为该模块准备一个模块级命名空间；在很多实现中，当前根函数对应栈帧中的 `locals` 与 `globals` 都会直接指向这同一个字典，用于承载顶层赋值和顶层函数定义形成的名称绑定。当 `MAKE_FUNCTION` 在这个环境中创建 `func` 时，会把该模块级 `globals` 字典一并绑定进新生成的函数对象中。于是，当我们执行 `mod_b.py` 后，得到的输出结果为 10 而非 999：虽然 `func` 是在 `mod_b.py` 中被调用的，但它执行 `LOAD_GLOBAL` 时，查找的仍然是自己在定义阶段绑定下来的 `mod_a` 模块全局命名空间，而不是调用点所在的 `mod_b` 命名空间。

因此，`locals`、`globals` 和 `localsplus` 的职责并不相同，也不能彼此替代。对于模块级根函数，对外可见的 `locals` 与 `globals` 可以指向同一个模块命名空间字典；但对于一般函数，保留独立的 `globals` 字段仍然是必要的，因为函数体中的 `LOAD_GLOBAL`、`STORE_GLOBAL` 需要据此访问该函数在定义阶段绑定的模块级全局命名空间，而不是调用点所在栈帧的局部环境。也正因如此，一个完整的 PVM 栈帧在理论上往往既要保存语义层的命名空间信息，也要保存实现层的高效槽位区。
