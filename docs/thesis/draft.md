...
就整体技术路线而言，本文 S.A.A.U.S.O VM 嵌入接口层中的整体架构与抽象接口组织部分参考了 V8 的设计思路。即在该层中，对外暴露 `Isolate`、`Context`、`Script`、`Value`、`Function`、`TryCatch` 等宿主可直接理解和使用的抽象概念，对内连接 VM 内核当中的执行层和对象系统，从而向嵌入方彻底隐藏 VM 内核的复杂实现细节与内部状态[16]。不过，本文系统并未直接照搬 V8 庞大的接口体系，而是围绕轻量级 PVM 的功能目标与实际需要，对相关设计进行了简化、迁移与适配，形成了一套轻量、易用的 API 集合。后文 4.6 将进一步说明这些对外抽象如何建立在前述 VM 内核能力之上。
