...
需要说明的是，正如第 1 章所述，现阶段 S.A.A.U.S.O VM 的目标是建立一个兼容 CPython 3.12 核心字节码模型、结构清晰且便于教学、裁剪或二次开发的最小 VM 系统闭环。因此，本章的测试将围绕验证系统的功能正确性与运行稳定性展开，而不是与成熟的工业级 PVM 系统进行大规模性能跑分比较。
...
本章围绕功能正确性、兼容性、运行时稳定性与 Embedder API 可用性，对现阶段 S.A.A.U.S.O VM 进行了系统测试与结果分析。测试结果表明，当前系统能够通过全部 438 个自动化测试用例。
