...
其中 `func` 函数读取变量 `x` 的操作是通过 `LOAD_GLOBAL` 字节码指令完成的。当我们执行 `mod_b.py` 后，得到的输出结果为 10 而非 999。这意味着当 `mod_a.py` 中的 `func` 函数与所在 Python 模块的全局命名空间发生了动态绑定；而当该函数被调用执行时，PVM 又会以该函数绑定的全局命名空间为准，来查找全局变量。在 CPython 的执行模型中，实现这种动态绑定全局命名空间的效果，同样依赖于函数对象在运行时阶段被动态地创建。
