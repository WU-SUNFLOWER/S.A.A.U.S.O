...
在 3.4.2 中，已经论述了 tracing GC 的总体思想，并引出了根集合与可达性的概念。在实现层面，S.A.A.U.S.O VM 的根集合并不单一，它既包括 `Isolate` 持有的关键运行时组件，也包括解释器调用栈、句柄机制、内建类型元信息、模块缓存和异常状态中的对象引用。
...
从这段代码可以看到，字典对象被分配后并不会立刻申请内部数据结构，而是先在 `DisallowHeapAllocation` 保护下完成最基本的字段初始化，将指针字段预填充为 null。这样，即使后续因再次分配而触发 GC，也不会因为未初始化指针而出错。因此，本文系统中的工厂函数均按先完成对象基础字段初始化，再执行后续依赖堆分配的步骤进行实现。

## 4.2 句柄机制实现

在 3.4.3 中，已经从总体设计角度论述了引入句柄机制的背景与动机。本节进一步说明句柄在 S.A.A.U.S.O VM 中的实际使用方式及其内部实现。
...
概括而言，S.A.A.U.S.O VM 把“维护 native 代码持有的堆对象引用”转化为“维护一组已知槽位”，从而解决了 3.4.3 中提出的问题。

## 4.3 对象系统实现

在 3.4.4 中，本文已经从总体设计角度说明了对象表示层的职责与思路。本节进一步讨论这一思路在 S.A.A.U.S.O VM 中的具体落地。
...
S.A.A.U.S.O VM 的嵌入接口采用“公共头文件 + API 桥接层 + VM 内核实现”三层结构。最外层是供宿主直接包含和调用的公共接口和抽象概念，中间层负责对接公共接口与 VM 内核能力，并把抽象概念转换为 VM 内核可理解的内部表示，最内层则是前文介绍过的解释器、运行时和对象系统等 VM 内部实现。这样的分层方式，使对外接口能够与 VM 内核保持相对稳定的边界关系。 

这种结构的主要优点在于，VM 的内部对象布局、GC 细节与解释器状态管理方式不会直接泄漏给外部调用者。宿主程序可以通过一组相对稳定的抽象概念使用虚拟机：其中，`Isolate` 表示一个独立的 VM 实例，`Context` 表示脚本运行的上下文环境，`Script` 表示待编译或待执行的脚本对象，`Value` 是宿主与脚本之间交换数据的统一抽象，`Function` 表示可由宿主或脚本发起调用的函数对象，`TryCatch` 则用于在宿主侧观测和处理 VM 抛出的异常。借助这些抽象概念，宿主程序无须了解和直接依赖内核实现细节，便能够完成对虚拟机核心能力的使用。

### 4.6.2 运行时容器

在 Embedder API 中，`Isolate` 作为 VM 实例与运行时容器提供给宿主程序。宿主程序首先初始化库并创建 `Isolate` 实例，再在其内部创建 `Context` 作为脚本运行时的上下文环境，最后通过 `Script` 接口编译和执行脚本。当宿主完成 Python 脚本执行并且不再需要使用 VM 时，可以直接销毁对应的 `Isolate` 实例。这样的设计既便于宿主程序使用和管理 VM 实例；同时由于 VM 内部已经建立了 `Isolate` 的概念，只需将其包装并作为对外接口暴露给嵌入方即可，这也简化了公共接口设计与实现的复杂度。

从宿主视角看，这套接口显著降低了宿主方的开发者接入 S.A.A.U.S.O VM 所需的理解成本：只要把握 VM 实例、上下文环境和脚本对象这三个核心概念，便能够完成基本的嵌入流程。
...
这种双向互操作能力，是 S.A.A.U.S.O VM 具备“脚本引擎”属性的重要体现。对本文而言，这一部分的意义并不在于单独展开接口细节，而在于说明前文构建的对象系统、函数调用机制与异常传播机制，不仅能够支撑 VM 内部执行，也能够被进一步封装为宿主程序可直接利用的外部能力。换言之，Embedder API 在本章中的作用，更接近于对 VM 内核可用性与工程封装能力的补充性验证。
