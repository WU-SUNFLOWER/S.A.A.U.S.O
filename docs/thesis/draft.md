...
第 3 章已经给出了 S.A.A.U.S.O VM 的层次划分、核心运行时容器以及核心层的总体设计与技术路线，并说明了具体的系统开发顺序。本章按照第 3 章给出的开发顺序，进一步说明这些设计在实际系统中的落地方式。本章的重点在于展示系统底层机制（如内存、对象模型）与高层机制（如解释器、模块）是如何通过具体的 C++ 结构和算法被一步步构建出来的。
...
为了进一步说明算法的执行过程，图 4.1 给出了一个具体的例子。

![Cheney 算法对象复制与转发流程图](./images/scavenge1.png)

![Cheney 算法对象复制与转发流程图续](./images/scavenge2.png)
图 4.1 Cheney 算法对象复制与转发流程图

如上图阶段 ② 所示，当系统扫描已复制的存活对象时，若发现某对象（如图中对象 A）已经持有了指向新位置的 Forwarding 指针，则说明该对象已被其他引用路径复制过。此时系统不会进行重复复制，而是直接修正当前引用，将其指向 Forwarding 指针所记录的新地址。这一机制是 Cheney 算法能够高效处理复杂对象图（包含多重引用或循环引用）的关键。

为了在代码层面支撑上述机制，代码 4-2 给出了 `ScavengerCollector` 与 `ScavengeVisitor` 的精简实现。前者负责组织阶段性的回收流程，后者负责对具体槽位中的对象执行复制、转发与引用更新。

```cpp
void ScavengerCollector::CollectGarbage() {
  ScavengeVisitor visitor(heap_);
  
  // 步骤 (2)：从根集合出发，将存活对象复制到 Survivor 区
  heap_->IterateRoots(&visitor);

  // 步骤 (3) & (4)：扫描 Survivor 区，进行广度优先遍历
  while (scan_page != nullptr) {
    Tagged<PyObject> object(scan_ptr);
    size_t instance_size = PyObject::GetInstanceSize(object);
    // 遍历该对象内部的引用槽位，将子对象也复制过去
    PyObject::Iterate(object, &visitor);
    scan_ptr += instance_size;
  }

  // 步骤 (5)：交换 Eden 与 Survivor 的角色
  heap_->new_space()->Flip();
}

void ScavengeVisitor::EvacuateObject(Tagged<PyObject>* slot_ptr) {
  // 核心机制 1：如果该对象已被复制过，直接返回其转发地址 (Forwarding Address)
  if (mark_word.IsForwardingAddress()) {
    *slot_ptr = mark_word.ToForwardingAddress();
    return;
  }

  // 核心机制 2：在 Survivor 区分配新空间，并拷贝对象数据
  ...
  // 核心机制 3：在原对象位置留下转发指针，并更新当前槽位的引用
...
引入统一工厂函数的主要目的，是收敛分配与初始化逻辑、封装堆分配细节，并避免初始化顺序不当而破坏 GC 安全性。对于可能因分配而触发 GC 的系统而言，若对象的指针字段尚未初始化完成便被扫描，就可能访问到无效引用。因此，本文系统中的工厂函数均按“先完成基础字段初始化，再执行后续依赖堆分配步骤”的顺序实现。至此，系统已经具备了“在受管堆上安全分配、扫描并回收对象”的最小内存管理闭环；下一节将继续解决 GC 存在时本地 C++ 代码如何安全持有这些对象的问题。
...
如图 4.2 所示，从实现本质上看，对象句柄 `Handle<T>` 当中并没有直接保存对象地址，而是保存了指向某个地址槽位的指针。读者可以把它理解为：本地 C++ 代码并不直接抓住“对象本体”，而是先抓住一个由 VM 统一管理的中间槽位。因此，当 VM 通过对象句柄访问对象时，实际上存在两次解引用：第一次对指向槽位的地址解引用，取出槽位中保存的对象地址；第二次再根据该地址访问位于虚拟机堆上对象的内容。
...
概括而言，S.A.A.U.S.O VM 把“维护散落在本地代码执行栈中的零碎堆对象引用”转化为“集中维护一组连续的已知槽位”，从而解决了 3.4.2 中提出的跨越 C++/受管堆边界的内存安全问题。至此，系统已经建立起“堆负责对象移动，句柄负责引用稳定”的协同关系；在此基础上，下一节才能进一步讨论统一对象系统如何组织对象状态与类型行为。
...
与此同时，Python 仍保留了高度动态的行为覆写能力。为兼顾语言层动态性与内部执行效率，现阶段系统采用“优先走虚函数表，必要时退化为字典查找”的**双层多态方案**。图 4.5 给出了这一机制的运作原理。
...
在 S.A.A.U.S.O VM 中，创建用户自定义类的过程是由 `Runtime_CreatePythonClass()` 函数完成的，图 4.6 给出了该函数的内部处理流程。
...
通过上述三个阶段的初始化流程，用户自定义类既能继承父类的高效 C++ 核心行为，又能完美兼容 Python 语义所允许的动态覆写能力。最终，`Runtime_CreatePythonClass()` 函数会向执行层返回一个状态完整、可正常使用的类型对象。至此，系统已经建立起后续执行层可统一依赖的对象语义底座；下一节将进一步说明，在这个底座之上，还需要先搭好哪些运行前提，解释器才能真正开始执行字节码。
...
对本文课题而言，重点研究对象是 PVM 后端，因此 S.A.A.U.S.O VM 在源码编译能力上直接封装了 CPython 的编译前端；关闭该复用后，PVM 后端仍可独立运行，但部分依赖源码编译前端的功能会受到限制。与此同时，系统也支持直接装载 CPython 编译器前端生成的 `.pyc` 代码对象二进制文件，从而为脚本执行提供源码与字节码两类输入入口。至此，解释器真正启动前所依赖的运行入口、内建环境、模块能力与代码输入来源都已经被准备完毕；下一节将沿着函数对象、栈帧与异常控制流的顺序，把真正的执行主链路闭合起来。
...
若把解释器主循环与异常处理路径放在同一张全局流程图中观察，则可得到图 4.7。可以看到，普通字节码 handler 的执行、`DISPATCH()` 宏的统一收口，以及异常发生后经由 `pending_exception_unwind` 进入异常表查找与栈展开路径，实际上共同构成了同一条解释执行主链路。换言之，异常并不是解释器之外的补丁逻辑，而是解释器在遇到特殊控制流时的内建分支。

![解释器主循环与异常处理流程图](./images/interpreter-loop-and-exception-process.png)
图 4.7 解释器主循环与异常处理流程图

至此，S.A.A.U.S.O VM 已经完成了解释执行主链路的闭环：前文准备好的对象系统与执行层基础设施，在这一节中被真正接入函数调用、字节码调度和异常传播路径之中。也正是在这一基础上，系统才有条件进一步向宿主程序暴露稳定的嵌入接口。
