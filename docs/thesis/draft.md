...
引入统一工厂函数的目的主要是为了收敛对象分配与初始化逻辑、封装堆分配细节，并避免上层代码因初始化顺序不当而出错。这在可能因分配内存而触发 GC 的系统中尤其重要：若对象的指针字段尚未初始化完成便触发 GC，扫描过程就可能访问到无效引用（即野指针）。因此，本文系统中的工厂函数均按先完成对象基础字段初始化，再执行后续依赖堆分配步骤的顺序实现，以避免初始化未完成的对象暴露给 GC。
...
代码 4-3 给出了上述机制的删节实现。可以看到，`Handle<T>` 负责保存并解引用槽位，而 `HandleScopeImplementer::CreateHandle()` 则负责完成“从原始对象地址申请槽位并写入对象地址”的过程。同时，`HandleScopeImplementer` 负责维护整片内存块的分配状态。
...
对本文系统来说，创建用户自定义类，本质上是同时完成 Python 语义层可见的类型对象建立，以及 `Klass` 中元信息、继承关系和虚函数表的初始化。其中 `Klass` 的初始化尤为重要，因为它是 4.3.2 中属性查找与 4.3.3 中核心行为分派得以正确工作的基础。代码 4-7 给出了这一过程的删节实现。
...
`ModuleImporter` 是模块子系统中核心导入逻辑的调度枢纽。在 `ModuleImporter` 中，系统首先调用 `ModuleNameResolver` 组件解析导入模块名中的相对导入符号，再进一步按照完整模块名进行分段导入。例如，如果用户想要导入的完整模块名为 `a.b.c`，则首先需要导入包 `a`，再导入子包 `b`，最后导入子包 `b` 中的模块 `c`。由于在 Python 中包被视作一种特殊的模块对象，因此这部分逻辑可以复用。进一步地，分段后导入具体包或模块时，系统会先检查缓存；若目标模块已经加载，则直接复用现有对象；否则再转入 `ModuleLoader` 执行路径查找和模块装载逻辑。
...
在建立起函数对象、栈帧和变量访问机制之后，就可以搭建解释器“逐条调度执行字节码指令”的主路径了。代码 4-13 给出了实际的 S.A.A.U.S.O VM 中，调度和控制流相关逻辑的删节实现。
