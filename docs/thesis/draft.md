...
需特别说明的是，为了避免 VM 系统在自举时发生对内建类型的循环依赖，现阶段的系统采用分阶段完成内建类型初始化的设计，即全体内建类型先完成各自 `PreInitialize()` 中较为简单、不可能依赖其他内建类型的初始化步骤，使得全体内建类型对 VM 内部逻辑来说最小可用；再分别执行 `Initialize()` 中较为复杂、可能依赖其他内建类型的初始化步骤，使得内建类型的 Python 语义得以完备。
