...
这意味着 PVM 在实现对象系统时，不能简单照搬静态语言中“字段偏移在编译阶段固定、方法入口在编译阶段预先绑定”的对象模型，而是需要采用字典容器的方式分别存储对象实例属性和类属性，并在属性读取时按一定顺序进行解析：通常先查实例自身持有的属性，再查类型对象及其继承链上的类属性，必要时再进入补救机制。只有这样，PVM 才能正确支持 Python 运行时的动态封装语义。

与此同时，方法绑定也是 Python 动态封装行为的一部分。当某个属性的查找结果来自类及其继承链，且该结果本身是函数对象时，PVM 需要在运行时把它与当前实例组合为“绑定方法”。当 Python 程序调用"绑定方法"时，PVM 会自动传入相应的对象实例作为方法函数的 `self` 参数。而如果某个函数只是被临时写入某个实例自己的属性字典中，那么它会被 PVM 视作一个普通属性值，并不会生成"绑定方法"。
...
在总体设计上，S.A.A.U.S.O VM 采用“对象实例数据与类型行为分离”的思路。对象实例主要负责保存字段数据，而与类型相关的名称、继承关系、核心行为等，则交由独立的类型描述结构承担。这样做的目的在于让对象表示和对象行为得以解耦。同时 VM 中需要建立统一的对象核心行为分派机制，以避免解释器和运行时为每一种对象类型编写大量特判分支。

在这种设计下，VM 中的 Python 对象需要由两部分共同进行描述：一部分是该对象在内存中的实体（包括对象头与对象实体数据），另一部分是该对象关联的类型元信息。前者回答“这个对象当前保存了什么状态”，后者回答“这个对象是什么类型、能够参与哪些行为分派”。这种结构既贴合 HotSpot、V8 等现代工业级 VM 的主流实现思路，也便于在二次开发时扩展新的内建类型。
...
这段代码揭示了当前 GC 逻辑的几个关键工程特征。首先，回收并不是从堆中“盲目扫描全部对象”开始，而是从根集合出发，把所有可达对象逐步复制到 Survivor 区。其次，复制完成后，Eden 区中原对象位置会被写入该对象被复制至的新内存地址（被称为 forwarding address）；后续若再遇到指向该对象在 Eden 区中旧位置的引用，系统不需要重复复制，只需读取 forwarding address 并让外部引用指向新地址即可。再次，`ScavengerCollector` 并未采用递归式的 DFS 遍历，而是以 Survivor 空间中的已复制对象作为一个隐式的 BFS 队列，按线性扫描方式不断推进，这种做法既避免了深递归，又无需开辟额外的内存空间。
...
在 3.4.4 中，已经提出了“对象实例数据与类型行为分离”的对象模型设计总思路。在具体实现上，S.A.A.U.S.O VM 中所有 Python 对象的实体都统一纳入 `PyObject` 体系；对象对应的类型元信息则由 `Klass` 体系统一描述。`PyObject` 更偏向对象实体的布局与字段组织，`Klass` 更偏向对象行为、类型关系和若干底层操作入口的组织。
...
然而，仅仅区分对象实体与类型元信息，还不足以回答第 2 章提出的全部问题。若要真正支持 Python 的动态封装语义，系统还必须进一步解决实例属性存放在哪里、类属性和方法如何组织、属性查找按什么顺序展开，以及方法在何时被绑定为可调用对象等问题。

### 4.3.2 实例属性、类属性与方法绑定

针对第 2.4.3 节提出的“字典化封装”问题，S.A.A.U.S.O VM 的实现并没有把所有属性都粗暴地堆入同一个容器中，而是将实例属性与类属性分开组织：实例对象通过 `PyObject::properties_` 持有自身的属性字典，类型对象对应的 `Klass` 则通过 `klass_properties_` 持有类属性字典。这样一来，“对象当前持有的具体状态”和“类型为所有实例共享的属性与方法”就在结构上被分开表示了。

与此同时，系统也没有假定所有类型实例都必须拥有实例级 `__dict__`。在对象创建阶段，工厂会根据 `Klass::instance_has_properties_dict()` 的配置决定是否为新对象分配实例属性字典；而在用户自定义类创建阶段，运行时又会显式将该标志置为 `true`。这种差异化策略使 PVM 既能够让用户类实例保留 Python 所要求的动态属性能力，也能够让一部分布局更稳定的内建类型维持更紧凑的对象表示。

代码清单 4-12 给出了实例属性字典、类属性字典以及实例 `__dict__` 分配策略的相关实现。
...
这段代码表明，所谓“Python 对象具有动态属性”在工程上并不是一句抽象描述，而是被落实为两项可观察的结构事实：其一，实例属性与类属性分别有独立的承载位置；其二，某个类型的实例是否拥有 `__dict__`，是由类型元信息显式控制的。也正因为如此，`int`、`str` 这类内建类型实例与用户自定义类实例，才能在同一套对象体系下表现出不同程度的动态属性能力。

属性容器确定之后，下一步就是回答属性查找与方法绑定如何在运行时落地。代码清单 4-13 给出了 `PyObjectKlass::Generic_GetAttr()` 与 `Generic_GetAttrForCall()` 的删节实现。
...
从该实现可以看到，S.A.A.U.S.O VM 中属性查找的主顺序与第 2.4.3 节的理论分析是相互对应的：首先尝试在实例字典中查找；若未命中，再沿实例所属类型的 MRO 序列查找类字典；若类字典命中的是函数对象，则在普通属性读取路径中通过 `NewMethodObject()` 构造绑定方法对象；若仍未找到，则继续尝试 `__getattr__`；若全部失败，最终抛出 `AttributeError`。这说明“实例状态、类行为、补救机制”在工程上确实被串成了一条明确的解析链。

更重要的是，这段实现也准确解释了第 2.4.3 节中提到的一个关键细节：为什么向实例字典中动态写入的函数值，通常不会被自动视为绑定方法。原因在于，实例字典命中后会直接返回结果，而 `MethodObject` 的创建只发生在沿 MRO 查找类属性并命中函数对象之后。换言之，在 S.A.A.U.S.O VM 中，方法绑定并不是对所有“看起来像函数”的属性值一视同仁，而是有明确触发条件的运行时行为。

此外，`Generic_GetAttrForCall()` 还为调用场景做了进一步优化。对于类字典中命中的 `PyFunction` 或 `NativePyFunction`，该路径会直接返回函数对象，并通过 `self_or_null` 单独回传接收者，而不必总是构造临时 `MethodObject`。这表明系统在保持 Python 语言语义的同时，也已经开始针对调用热路径做工程优化。
