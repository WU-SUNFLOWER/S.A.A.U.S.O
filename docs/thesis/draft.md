...
本章中并不会完整复述 Python 语言的所有功能细节，而是围绕 S.A.A.U.S.O VM 所支持的 Python 语言核心功能，分析并说明一个兼容 CPython 3.12 字节码模型的轻量级 PVM 通常需要具备哪些关键能力，以及这些能力的实现原理是什么，从而为本文系统的实际实现提供理论基础。
...
在 CPython 的执行模型中，字节码程序通常由代码对象（code object）描述。代码对象不仅保存字节码指令序列，还会记录符号名列表、常量列表等元信息。换句话说，代码对象相当于一段 Python 程序的静态描述。至于 Python 程序运行过程中的动态状态，则在将要探讨的 PVM 栈帧（stack frame）中进行维护，它在后文 2.3.2 中将会详细展开。
...
需要指出的是，实际 PVM 的执行过程并不只是维护一个操作数栈那么简单。为了支持函数、异常传播、模块加载以及各种更加复杂的语言功能，PVM 通常还需要维护函数调用堆栈、变量命名空间和异常状态（exception state）等状态信息，并且依赖语言运行时语义子系统（runtime）提供服务。

综上所述，一个可运行的 PVM 总体上必须围绕代码对象、栈帧、程序计数器和运行时服务建立稳定的执行主路径。第 3 章对本文系统中执行层的划分，以及第 4 章中解释器的实际实现，都是基于这一点展开的。
...
分析编译结果可知，在运行时这段代码时 PVM 需要反复完成两件事：一是判断循环条件是否成立，二是在循环体执行完毕后重新回到条件检测位置。前者依赖条件跳转字节码（如 `POP_JUMP_IF_FALSE`、`POP_JUMP_IF_TRUE` 等），后者依赖后向跳转字节码（如 `JUMP_BACKWARD`）。因此，从 PVM 的角度看，循环并不是一种独立机制，而是“条件判断 + 跳转”的组合。

在 Python 中，控制流判断还涉及真值语义（truthiness）。数字、字符串、容器乃至用户自定义对象，都允许被 PVM 解释为真或假。这意味着 PVM 在执行条件跳转前，往往还需要借助运行时规则先将对象转换为逻辑上的真值结果。换句话说，控制流虽然表现为程序计数器的变化，但背后仍离不开对象系统与运行时语义的参与。

对本文后续 PVM 的实现而言，这一点非常关键。PVM 不仅需要支持基本跳转字节码，还必须让这些跳转能够和对象真值判定等运行时能力协同工作。否则，即便单独实现了跳转指令，也无法真正实现 Python 程序中的控制流行为。
...
下面通过一个例子进一步说明 CPython 执行模型中采用这种动态创建函数对象机制的必要性。Python 允许为函数形参设置默认值，而且该默认值可以来自运行时阶段才确定的对象，例子见代码 2-8。
```python
default_k = 3
...
从这段指令序列中可以看出，创建带有默认值参数的函数时，首先由 `BUILD_TUPLE` 指令先动态构造默认值元组并压栈，随后 `MAKE_FUNCTION` 指令再把该元组与代码对象一并绑定进最终得到的函数对象中。这说明 Python 中的函数不能被简单看作“静态代码片段”，而必须被建模为运行阶段动态创建的对象。

另外，函数对象在被创建时，还需要绑定当前 PVM 正在使用的全局命名空间。这将会在 2.3.2 中结合函数调用机制进一步说明。

综上所述，PVM 中函数对象必须同时保存代码对象、默认参数、闭包信息以及定义时环境等元数据。第 4 章中将会给出函数对象在本文系统中的实际实现。
...
综上所述，PVM 栈帧中需要同时提供 `locals`、`globals` 和 `localsplus` 字段。它们的职责并不相同，也不能彼此替代。对于模块级根函数，对外可见的 `locals` 与 `globals` 可以指向同一个模块命名空间字典；但对于一般函数，对应栈帧中保留独立的 `globals` 字段仍然是必要的，因为函数体中的 `LOAD_GLOBAL`、`STORE_GLOBAL` 需要据此访问该函数在定义阶段绑定的模块级全局命名空间，而不是调用点所在栈帧的局部环境。
...
综上所述，在 CPython 3.12 的执行模型中，闭包机制并不是简单在普通局部变量的基础上打补丁，而是一套需要由编译器前端、单元对象间接层和专用字节码指令共同协作完成的完整机制：编译器前端先区分普通局部变量和自由变量，并生成 `MAKE_CELL`、`LOAD_CLOSURE`、`COPY_FREE_VARS`、`LOAD_DEREF`、`STORE_DEREF` 等专用字节码指令，再由 PVM 在运行时据此完成自由变量的捕获、跨函数传递和读写。
...
因此为了抹平这种差异，在本文所实现的 S.A.A.U.S.O VM 中，选择将普通函数和内建函数采用统一的 Python 对象形态进行表示；在虚拟机系统实际执行函数调用操作时，才予以区分并执行不同的实际调度操作。在 4.5.5 中将会讨论实际的实现细节。
...
另外值得一提的是，在 Python 中不同内建类型的实例属性承载能力是不同的。对于绝大部分具备独特内存布局的内建类型，如 `int`、`float`、`str`、`list`、`tuple` 和 `dict`，Python 在语言设计层面并不允许用户向它们的实例动态添加属性，因为它们的实例并不具备属性容器。而对于函数、模块或用户自定义类实例，则通常还需要为其提供属性容器，以支持运行时动态增加属性。因此，PVM 在实现对象机制时，应当允许不同类型在动态属性能力上存在差异。
...
本章首先探讨了 PVM 的执行原理，指出从执行模型上来看 PVM 后端在本质上是一个虚拟的栈机；然后逐一分析为了实现 Python 语言的核心功能，PVM 后端应当具备的能力，以及这些能力的实现原理是什么。
...
对于一个以托管对象为核心的 PVM 而言，虚拟机堆是供对象系统、解释器和运行时语义等更加上层的子系统得以运作的前提。S.A.A.U.S.O VM 在这一部分的落地实现，并不是一步到位完整复刻工业级 VM 高度复杂的内存分配与管理系统，而是优先建立一个功能上能够支撑 VM 上层主要功能闭环、架构上易于未来拓展的轻量级内存托管与垃圾回收系统。
...
在本文系统中引入工厂函数的概念，主要有两个目的。其一，可以避免对象分配与初始化逻辑散落在 VM 更上层的业务逻辑中，或者在 VM 更上层中出现重复造轮子的问题。其二，提供统一的抽象工厂函数对 VM 堆中的内存分配接口进行封装，可以避免 VM 堆直接向更上层逻辑暴露接口细节，从而强化系统的分层架构边界。其三，将对象的分配与初始化逻辑收敛到统一的工厂函数中完成，可以避免 VM 更上层的业务逻辑在自行创建和初始化对象时，因操作顺序不当而引发错误。
...
在 3.4.3 中，已经从 VM 总体设计的角度，论述了引入句柄机制层的背景与动机，并说明了其总体思路借鉴了 V8 的设计。在本节中，将进一步从实际 S.A.A.U.S.O VM 系统实现的角度，介绍对象句柄在实际系统中的使用方法，及其内部原理。
...
需要强调的是，句柄机制是 S.A.A.U.S.O VM 中解释器、运行时和对象工厂等高层逻辑都必须依赖的基础设施。只有让 VM 中的所有高层逻辑都统一依赖句柄机制，才能将 VM 内部高层逻辑中所有堆上对象的临时引用，统一纳入虚拟机堆系统和 GC 逻辑的统一管控，而不会演变为难以追踪的 C++ 裸指针。
...
可以进一步指出的是，在现阶段的 S.A.A.U.S.O VM 实现中，这段代码中的 GC 访问器，即为 4.1.4 中提及的 `ScavengeVisitor`。如代码 4-10 所示，在 `ScavengeVisitor::VisitPointers()` 当中，会进一步调用 `ScavengeVisitor::EvacuateObject()` 将槽位所指的对象复制至 Survivor 空间，并在槽位中写入复制后对象的新地址。`ScavengeVisitor::EvacuateObject()` 的内部实现代码，同样已经在 4.1.4 给出过了。
...
在 3.4.4 中，本文已经从总体设计角度说明了对象表示层的职责与设计思路，并交代了其中“对象实例数据与类型行为分离”的总体思路参考了 HotSpot JVM 的设计经验。本节将进一步讨论这一思路在 S.A.A.U.S.O VM 中的具体落地实现。
...
在 2.4.2 中已经指出，一个最小可用的 PVM 必须先提供若干基础内建类型，才能承载后续的运行时语义。从实现角度看，这些内建类型并不是由用户 Python 代码通过 `class` 语句在运行时临时创建出来的，而是由 VM 在 C++ 侧预先定义好的对象类型。也就是说，PVM 既要为它们提供各自的实例对象布局，又要为它们准备对应的类型元信息与核心行为实现。

在 S.A.A.U.S.O VM 中，这种“预先定义”的含义主要体现在两层。第一层是对象实体层：对于位于堆上的内建对象，系统会在 C++ 中定义继承自 `PyObject` 的具体对象类，例如 `PyString`、`PyList`、`PyDictKlass` 等，以承载各自独特的字段布局。第二层是类型元信息层：系统还会为每一种内建类型定义一个对应的 `Klass` 子类，例如 `PyStringKlass`、`PyListKlass`、`PyDictKlass` 等，用于描述该类型的名称、继承关系、实例布局特征以及核心行为分派入口。需要指出的是，并非所有内建值都一定以普通堆对象的形式出现；例如，在 4.1.1 中已经提到当前系统中的整数采用标记指针的方式进行表示，但它仍通过 `PySmiKlass` 被统一纳入整个对象系统。
...
从这段代码中可以看到，`PyList` 在 `PyObject` 内存布局的基础上，又引入了一个整型字段用于记录列表的长度，以及一个引用字段用于指向实际装载列表元素的定长数组。

对单个内建类型而言，其初始化过程与前文介绍的统一对象模型是完全衔接的。在 VM 启动阶段，内建类型的 `Klass` 实体会被创建并由 VM 内部代码完成元信息填写、MRO 序列的计算、虚函数表初始化、在类型字典中安装内建方法等操作。此外，VM 还需要为内建类型创建对应的 Python 类型对象，并且在类型对象与 `Klass` 实体之间建立双向绑定的关系。

以 `list` 类型为例，代码 4-21 给出了这一过程的。

```cpp
void PyListKlass::PreInitialize(Isolate* isolate) {
  ...
  // 填写基础元信息，
  // 例如将 instance_has_properties_dict 字段设置为 false。
...
需特别说明的是，为了避免 VM 系统在自举时发生对内建类型的循环依赖，现阶段的系统采用分阶段完成内建类型初始化的设计，即全体内建类型先完成各自 `PreInitialize()` 中较为简单、无需依赖其他内建类型的初始化步骤，使得全体内建类型对 VM 内部逻辑来说最小可用；再分别执行 `Initialize()` 中需要依赖其他内建类型的复杂初始化步骤，使得内建类型的 Python 语义得以完备。
...
与此同时，本章中的介绍也表明，S.A.A.U.S.O VM 的工程价值不但体现在单个功能模块的设计与实现，更体现在这些功能模块被统一组织在一个清晰的分层系统架构中，并形成稳定的依赖与协作关系。即便对于本章中未作大篇幅展开的 Embedder API，也并非另起一套独立体系，而是建立在 VM 内核的基础能力之上的。

基于现阶段 S.A.A.U.S.O 的实现基础，下一章将进一步通过测试与结果分析，对系统的功能正确性、兼容性和当前表现进行验证。

# 第5章 S.A.A.U.S.O VM 测试与结果分析

本章将围绕功能正确性、字节码模型兼容性、运行时稳定性以及 Embedder API 可用性四个方面，对现阶段的 VM 系统进行功能测试与结果分析。

需要说明的是，正如第 1章所述，现阶段 S.A.A.U.S.O VM 的目标是建立一个兼容 CPython 3.12 核心字节码模型、结构清晰且便于教学、裁剪或二次开发的最小 VM 系统闭环。因此，本章的测试重点将放在验证系统的功能正确性与运行稳定性上，而不是与成熟的工业级 PVM 系统进行大规模性能跑分比较。
...
需要首先说明的是，虽然 Python 官方在 CPython 的开源代码中提供了非常丰富的单元测试用例，但其中的大量测试用例涉及到了超出本次课题研究范围的 Python 语言高阶功能，对这些用例逐一进行筛选或修改的时间成本比较大。因此基于 5.1 中介绍的测试目标，现阶段 S.A.A.U.S.O VM 系统基于 Google Test 自动化测试框架自行维护了两套单元测试集，而没有直接使用 Python 官方的测试用例集。
...
这份统计结果说明，现阶段 S.A.A.U.S.O VM 已经具备了较为系统化的自动化回归基础，因而不需要通过手工演示的方式逐一验证功能。而在 5.3 和 5.4 中，还将进一步说明这些测试用例中具体覆盖的内容。

### 5.2.2 功能测试的执行

现阶段 S.A.A.U.S.O VM 已经将 Google Test 框架以及全部的单元测试用例集成于项目当中。执行单元测试前，只需运行项目根目录下的 `build.sh` 脚本文件，即可构建完整的 VM 内核与单元测试执行程序。

构建完毕后，分别运行针对 VM 核心的测试程序 `ut.exe` 与针对 Embedder API 的测试程序 `embedder_ut.exe`，即可批量执行单元测试用例并在命令行终端看到测试结果。

### 5.2.3 评价口径说明

需要特别指出的是，本章中的“结果分析”主要围绕以下三个口径展开。

其一，语义正确性优先。也就是说，本文首先关注“是否正确实现目标语义”，而不是仅以运行时间长短评价实现优劣。对于一个仍在持续演进的轻量级 PVM 来说，语义偏差通常比执行速度更值得优先纠正。

其二，兼容性评价聚焦于“核心字节码模型闭环”。本文并不宣称当前系统已经完整兼容 CPython 3.12 的全部语义，而是聚焦于前文明确讨论过的控制流、函数调用、闭包、对象模型、异常展开、模块导入与宿主互操作等核心能力。

其三，性能与资源表现采用“工程可用性”视角进行解释。也就是说，本文会报告当前测试回归的执行规模与耗时，并结合 GC 压力用例、递归用例和跨模块用例讨论系统当前表现，但不会在缺乏统一硬件对照与大规模基准套件的前提下，给出夸大的性能结论。
...
函数调用与闭包的实现比较容易出现错误，因此本文系统在这一部分设置了较为细致的单元测试。现阶段测试已经覆盖位置参数、关键字参数、默认参数、`*args/**kwargs` 展开、参数错误时抛出异常、方法调用时 `self` 参数注入、递归调用，以及自由变量捕获与闭包访问等场景。
...
在运行时基础设施方面，本文尤其关注句柄系统与 GC 机制的协同是否可靠。为此，当前项目构建了针对局部句柄、长期句柄、跨线程边界、GC 根遍历、移动后引用更新以及 GC 压力场景的测试。

例如，在 `gc-unittest.cc`、`gc-interpreter-stress-unittest.cc`、`global-handle-unittest.cc`、`handle-unittest.cc` 等源文件中，重点验证了对象在经历
 Scavenge GC 后引用仍然可达、句柄槽位能够随对象移动而被更新，以及 VM 在 GC 压力条件下不会错误丢失活对象。与此同时，还引入了若干解释器层的测试用例，用于确认内建对象在显式 GC 压力下仍能保持功能正常。

### 5.4.2 Embedder API 可用性验证

除了 VM 内核本身，Embedder API 也是本文的重要验收对象。当前 `test/embedder/` 共包含 7 个测试源文件、46 个 GTest 用例，主要包括 `Isolate` 创建与销毁、`Context` 创建与使用、脚本编译与执行、宿主与脚本的双向互操作、TryCatch 异常捕获等内容。

从测试内容看，这部分验证不仅覆盖了对外接口能否正常使用，还覆盖了接口在被嵌入方错误使用时是否及时拦截。例如，若宿主在未正确创建 `Isolate` 的情况下调用 API，或者尝试跨线程非法访问 `Isolate`，预期虚拟机在 debug 版本中会立即抛出断言错误。

## 5.5 测试结果总结与分析

### 5.5.1 测试结果

在本次论文撰写阶段的完整测试中，首先成功构建了 VM 内核与单元测试执行程序；随后分别运行核心 VM 测试与 Embedder API 测试。最终得到的自动化测试结果如表 5-2 所示。
...
从表 5-2 可以看到，现阶段 S.A.A.U.S.O VM 在一次完整的构建与回归测试中，438 个自动化测试用例全部通过，没有出现构建失败、链接失败或测试用例失败的情况。对一个处于轻量级实现阶段、但已经包含解释器、对象系统、GC、异常、模块与嵌入接口等多个核心子系统的 PVM 而言，这一结果说明其当前工程状态已经具备较好的整体一致性。

### 5.5.2 结果分析

结合第 5.3 节与第 5.4 节中的覆盖情况，可以从以下三个层面对结果进行分析。

第一，从功能正确性角度看，当前系统已经围绕“可运行 Python 程序”所需的核心能力建立了较完整的回归网。无论是控制流、函数调用、闭包、对象系统、内建类型、异常机制还是导入系统，均有对应专项测试，而不是仅依赖若干冒烟样例。因此，测试结果表明本文第 4 章给出的系统实现已经正确实现了 Python 语言的核心功能子集。

第二，从兼容性角度看，当前系统虽然尚未覆盖现代 Python 语言的全部功能特性，但围绕“兼容 CPython 3.12 核心字节码模型”这一论文目标，已经完成了较有代表性的主路径验证。尤其是闭包、异常表展开、模块导入、方法绑定、自定义类等 Python 核心语言能力均通过测试验证，说明本系统并非是只能运行简单表达式或控制流的 demo，而是初步具备了处理真实 Python 程序关键语义的能力。

第三，从稳定性角度看，当前测试结果不仅说明系统所实现的 Python 语言功能的正确性，同时也说明本文系统的稳定性。特别地，通过压力测试用例，可以证明现阶段本文系统的解释器、GC 机制与对象句柄机制等核心设施是稳定且可控的。这对 PVM 系统来说尤为重要，因为一个可实际使用的 PVM 系统，不但需要能够正确运行，更应该确保稳定运行。

### 5.5.3 当前局限

尽管测试结果总体积极，但仍应当保持客观。首先，现阶段测试重点仍然是功能正确性与核心语义闭环验证，尚未建立面向大规模 Python 基准程序集的系统化性能评测框架，因此本文不宜据此对执行效率作过度延伸解释。其次，当前自动化测试虽然已经覆盖大量核心能力，但对于生成器、协程、更完整的描述符语义、完整 importlib/元路径机制以及老生代回收等内容，还缺少与之相匹配的专项测试。最后，本章记录的时间数据主要用于说明当前回归规模和工程执行成本，尚不能替代严格控制硬件环境后的基准性能实验。

因此，对现阶段 S.A.A.U.S.O VM 更为准确的评价应是：它已经通过较大规模自动化回归证明了自身在核心语义与工程闭环上的可用性，但若要进一步证明其在更广泛 Python 程序上的兼容能力和性能表现，仍需要在后续工作中持续扩展测试集与基准评测体系。

## 5.6 本章小结

本章围绕功能正确性、兼容性、运行时稳定性与 Embedder API 可用性，对现阶段 S.A.A.U.S.O VM 进行了系统测试与结果分析。测试结果表明，当前系统已经形成由核心 VM 单元测试、Embedder API 单元测试和 Python 脚本回归样例共同组成的分层测试体系，并在一次完整构建回归中通过了全部 438 个自动化测试用例。

更重要的是，这些测试并不是孤立罗列的数字，而是与前文第 2 章至第 4 章中的理论分析和工程实现形成了对应关系：控制流、函数调用、闭包、对象模型、异常机制、模块导入与嵌入接口等关键能力，均已经在测试层面得到验证。这说明本文所实现的 S.A.A.U.S.O VM 已经具备了一个轻量级 Python 虚拟机应有的最小可用闭环，并为后续围绕更完整 Python 语义、更强兼容性与更高执行效率的持续演进奠定了基础。
