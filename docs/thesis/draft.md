...
之后，PVM 需要查找并将控制流转向 Python 程序中与异常发生点相匹配的异常处理器。这个过程是 PVM 通过栈展开（stack unwinding）完成的。
...
对于一个以托管对象为核心的 PVM 而言，虚拟机堆是更对象系统、解释器和运行时语义等更加上层的子系统得以运作的前提。S.A.A.U.S.O VM 在这一部分的落地实现，并不是一步到位完整复刻工业级 VM 高度复杂的内存分配与管理系统，而是优先建立一个功能上能够支撑 VM 上层主要功能闭环、架构上易于未来拓展的轻量级内存托管与垃圾回收系统。
