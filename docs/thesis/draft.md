...
总的来说，在 CPython 3.12 的执行模型中，编译器前端会负责区分普通的函数局部变量、单元变量（cell variable）和自由变量（free variable）。围绕这些变量，编译器前端会进一步生成一组专门服务于闭包机制的字节码指令与参数标记，例如：对外层函数中需要被共享的局部变量生成 `MAKE_CELL`，在创建内部函数前通过 `LOAD_CLOSURE` 收集相应单元对象，并在 `MAKE_FUNCTION` 的参数中标记该函数带有闭包环境；当内部函数真正被调用、建立新栈帧后，又通过 `COPY_FREE_VARS` 将其绑定的自由变量单元对象注入新栈帧的自由变量区域。PVM 正是在这些编译器前端生成的字节码与参数信息的驱动下，实现对闭包机制的正确处理的。

具体来说，为了正确实现闭包机制的效果，CPython 中引入了单元对象（cell object）的概念。对于自由变量，PVM 并不会直接将变量值保存进外部函数栈帧的局部变量区，而是会通过 `MAKE_CELL` 字节码指令在其局部变量区中创建一个单元对象，其中保存对变量值本身的引用。随后，在内部函数被创建之前，PVM 还会借助 `LOAD_CLOSURE` 把这些单元对象压入求值栈，并由 `MAKE_FUNCTION` 一并绑定进新生成的函数对象中。当内部函数真正被调用时，`COPY_FREE_VARS` 又会把这些单元对象重新注入新栈帧的自由变量区域，即让内部函数继续持有对同一个单元对象的引用，从而实现对自由变量值的持续持有。这解决了问题（2）。
...
综上所述，闭包机制并不是简单在普通局部变量的基础上打补丁，而是一套需要由编译器前端、单元对象间接层和专用字节码指令共同协作完成的完整机制：编译器前端先区分变量类别并生成 `MAKE_CELL`、`LOAD_CLOSURE`、`COPY_FREE_VARS`、`LOAD_DEREF`、`STORE_DEREF` 等相关字节码，再由 PVM 在运行时据此完成自由变量的捕获、跨函数传递和读写。
