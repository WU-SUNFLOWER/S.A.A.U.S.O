...
对本文系统来说，创建用户自定义类的过程，本质上是创建供 VM 内部使用 `Klass` 实体以及暴露给 Python 程序语义层面的类型对象的过程。

在用户自定义类创建时，解释器会调用入口函数 `Runtime_CreatePythonClass()`。该函数中的核心步骤可概括如下：
（1）构造新的 `PyTypeObject` 与 `Klass`，为 `Klass` 设置类名、类属性字典和基类列表（supers list）；若未显式给出基类，则默认补入 `object` 作为父类。
（2）把 `instance_has_properties_dict` 置为 `true`，从而保证该类的实例默认具备属性字典。
（3）进一步调用新创建 `Klass` 实体的 `OrderSupers()` 和 `InitializeVtable()` 方法。前者用于调用 C3 线性化算法以根据基类列表计算出类型的，并写入 `Klass` 实体内部保存。后者用于初始化类型的虚函数表，其中会进一步调用的关键逻辑 `KlassVtable::UpdateOverrideSlots()`，已经在 4.3.3 中介绍过了。
