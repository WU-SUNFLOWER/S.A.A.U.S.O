...
如图 3.1 所示，如果进一步按职责细分，按照自底向上的单向依赖原则，VM 内核可以进一步概括为如下几层：
...
在 VM 内核之外，S.A.A.U.S.O VM 还提供独立的嵌入接口层，对外暴露 `Isolate`、`Context`、`Script`、`Value`、`Function`、`TryCatch` 等宿主可直接理解和使用的抽象，对内则通过桥接逻辑连接 VM 内核当中的执行层和对象系统。该层的核心目标，是在不泄漏内部实现细节的前提下，为宿主程序提供清晰且便于接入的脚本引擎抽象接口。
