...
从这一类测试可以看到，在大量基础控制流、解释器调度和容器操作叠加的场景下，现阶段 S.A.A.U.S.O VM 的执行效率与成熟工业级 PVM 实现之间仍存在较明显差距。与 CPython 3.12 相比，当前系统在该测试中的平均耗时约为其 3 倍；即便与较早版本的 CPython 3.5 相比，也仍然偏慢。
