...
这意味着，PVM 在实现变量读写时，不能简单类比 C/C++ 等语言，把“读取变量”和“写入变量”理解为对单一内存位置的直接访问，而必须同时处理两层问题。第一层是 Python 语义层的命名空间映射，即变量名最终应当在什么作用域中被解释并绑定；第二层则是解释器为提高执行效率而采用的内部存储结构，例如以数组槽位形式组织的局部变量区。

在现代 CPython 3.12 的执行模型中，这两层问题通常并不完全重合。对于单个 Python 文件的顶层代码，也就是可视作“根函数”的那部分代码，PVM 往往直接在字典形式的命名空间中完成名称读写，因此编译器更可能生成 `LOAD_NAME`、`STORE_NAME` 等按名字解析的字节码。此时，当前栈帧中的 `locals` 与 `globals` 在逻辑上都指向该模块的命名空间，它们可以重合。相对地，对于一般的 Python 函数，形参与普通局部变量通常会被预先编排进一个按槽位顺序访问的局部变量区，解释器随后通过 `LOAD_FAST`、`STORE_FAST` 直接按下标访问这些槽位，而不再在每次读写时都进行字典查找。

与此同时，函数体对模块级全局变量的访问又是另一条路径。若函数内部需要读取其所在模块中的全局名称，编译器通常会生成 `LOAD_GLOBAL`，解释器在运行时会先查该函数绑定的全局命名空间，再查内建命名空间；若函数体显式使用 `global` 声明并对某个名称赋值，则相应写操作会通过 `STORE_GLOBAL` 落到该函数绑定的全局命名空间之中。至于闭包场景，自由变量的读写则进一步通过 `LOAD_DEREF`、`STORE_DEREF` 配合单元对象（cell object）完成。

对本文后续 PVM 的实现而言，这一机制具有直接影响。其一，S.A.A.U.S.O VM 中的栈帧既需要保存与 Python 语义对应的 `locals`、`globals` 等命名空间信息，也需要保存便于高效访问的局部变量槽位区；其二，顶层脚本需要提供模块级全局命名空间；其三，函数对象需要绑定其定义时所属的全局命名空间；其四，闭包函数还需要额外保存被捕获的自由变量。换言之，变量机制并不是 PVM 中的一个局部语法问题，而是贯穿代码对象、栈帧、函数对象和运行时行为的共同基础。
...
对于栈帧中的“局部环境”本身，还需要作进一步区分。站在 Python 语义层面，栈帧至少要能够表达当前函数可见的局部命名空间以及该函数按词法作用域绑定的全局命名空间；但站在解释器实现层面，若仍让所有局部变量都通过字典按名字查找，则执行代价会过高。因此，在现代 CPython 3.12 的执行模型中，除了 `locals`、`globals` 这类字典形式的命名空间外，栈帧内部通常还会维护一个按固定顺序组织的局部变量槽位区。在后文中，本文将沿用项目代码中的术语，把它称为 `localsplus` 数组。
...
与此同时，`locals`、`globals` 和 `localsplus` 的职责并不相同，也不能彼此替代。对于模块级根函数，对外可见的 `locals` 与 `globals` 可以指向同一个模块命名空间字典；但对于一般函数，保留独立的 `globals` 字段仍然是必要的，因为函数体中的 `LOAD_GLOBAL`、`STORE_GLOBAL` 需要据此访问该函数在定义阶段绑定的模块级全局命名空间，而不是调用点所在栈帧的局部环境。也正因如此，一个完整的 PVM 栈帧在理论上往往既要保存语义层的命名空间信息，也要保存实现层的高效槽位区。
