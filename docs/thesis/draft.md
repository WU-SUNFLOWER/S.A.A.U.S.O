...
在介绍句柄机制的内部实现之前，先说明它在 VM 日常开发中的典型用法。总体上，开发者很少在高层逻辑中直接使用 4.1.1 中介绍的 `Tagged<T>` 传递堆对象引用，而是优先使用 `Handle<T>`；当某段逻辑可能临时创建一批新的对象引用时，通常还需要先建立相应的句柄作用域，即 `HandleScope` 或 `EscapableHandleScope`。
...
需要强调的是，句柄机制是解释器、运行时和对象工厂等高层逻辑共同依赖的基础设施。只有统一通过句柄持有堆对象引用，才能把 native C++ 代码中的临时引用纳入 GC 机制的统一管控，而不演变为难以追踪的裸指针。
...
在这段代码中，系统首先创建新的类型对象和 `Klass` 并完成绑定；随后把类名、类属性字典以及基类列表写入 `Klass`；若用户没有显式声明父类，则自动补入 `object` 作为默认基类。此外，还会将 `instance_has_properties_dict_` 设为 `true`，以确保用户自定义类实例默认具备属性字典。
...
S.A.A.U.S.O VM 并没有把“执行一段脚本”设计为直接调用解释器底层方法，而是先通过执行门面层统一收口，再转入具体执行逻辑。这样既避免其他模块直接耦合解释器内部接口，也便于脚本执行、模块执行和函数调用共享一致的入口。

与此同时，执行层还承载了许多不适合直接写入解释器主循环的高层语义逻辑，例如类属性查找和用户自定义类型创建等。这些能力以 `Runtime_` 接口统一提供，以减轻解释器负担。
...
从这段代码可以看出，主脚本执行并不是简单把目标脚本函数交给解释器，而是还要处理默认全局命名空间的创建、`__name__ = "__main__"` 的注入，并最终进入统一的 `Call` 入口。把这些上下文准备留在 `Execution` 门面层统一组织，能够使解释器只专注于底层执行逻辑。
...
代码 4-33 给出了 `STORE_GLOBAL/LOAD_GLOBAL` 和 `STORE_FAST/LOAD_FAST` 的删节实现。它们与 2.3.2 中的理论分析基本一致，因此不再展开。
