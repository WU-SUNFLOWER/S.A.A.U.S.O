...
Python 语言凭借其简洁优雅的语法、动态类型特性以及庞大的第三方生态系统，已在当今 Web 开发、数据科学、人工智能及自动化运维等领域得到广泛应用。Python 作为一种跨平台的、动态类型的高级编程语言，其执行依赖于 Python 虚拟机（Python Virtual Machine，PVM）。
...
本课题所实现的 S.A.A.U.S.O VM 的实现已开源，相关链接见附录 A。
...
综合来看，业界现有的 Python 编程语言实现方案，在"轻量化、易嵌入、保持较强 CPython 兼容性、实现层面可读可维护"这几个维度之间往往存在权衡空白。本课题旨在以教学以及嵌入场景为目标，开发一个名为 S.A.A.U.S.O 的 PVM 后端系统，实现对 CPython 3.12 字节码的兼容解释执行，并给出清晰、易理解的分层架构方案与易裁剪、易定制修改的轻量级工程实现。

### 1.3.1 研究内容

- 设计并实现一个基于 C++20 的轻量级 PVM 后端，支持一个 Python 语言的核心功能子集，兼容 CPython 3.12 字节码执行模型；
- 提供易于嵌入的 API，使其可作为脚本引擎集成到宿主应用；
- 在保证正确实现 Python 语言核心功能的前提下，保持轻量级的项目规模与清晰的分层架构，便于教学与二次开发。
