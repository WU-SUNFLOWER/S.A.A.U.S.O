...
这意味着，PVM 在实现变量读写时，不能简单类比 C/C++ 等语言，把“读取变量”和“写入变量”理解为对单一内存位置的直接访问，而必须结合当前局部命名空间、全局命名空间、内建命名空间和闭包环境，首先完成对变量的查找，再执行实际的读写操作。具体到 CPython 3.12 的执行模型，局部变量、全局变量和自由变量在 PVM 中具有不同的查找逻辑，也对应不同类别的字节码访问方式，例如 `LOAD_NAME/STORE_NAME`、`LOAD_FAST/STORE_FAST`、`LOAD_GLOBAL/STORE_GLOBAL` 以及 `LOAD_DEREF/STORE_DEREF`。

对 PVM 的设计与实现而言，这一机制具有直接影响。其一，PVM 中的栈帧需要维护局部执行环境；其二，顶层脚本需要提供模块级全局命名空间（即每个 Python 代码文件会被视作一个模块，每个模块都拥有独立的全局命名空间）；其三，函数对象需要绑定其定义时所属的全局命名空间；其四，闭包函数还需要额外保存被捕获的自由变量。换言之，变量机制并不是 PVM 中的一个局部语法问题，而是贯穿代码对象、栈帧、函数对象和运行时行为的共同基础。关于这些不同字节码访问路径与栈帧内部变量环境之间的关系，将在 2.3.2 中结合函数调用进一步展开。
