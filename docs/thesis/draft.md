...
首先需要说明的是，本文的研究主线是设计并实现一个轻量级 PVM 后端系统，而不是构建高性能虚拟机。因此受课题范围和系统开发时间的限制，当前项目中并没有建立包含多类基准程序、重复测量、统计分析和硬件环境控制的规范化性能测试框架，当前系统也并未针对虚拟机性能进行深入优化。本节的目的不是给出严格意义上的基准结论，而是选取几类较为典型、实现成本可控且能够反映不同执行路径特征的 Python 程序，与 CPython 3.5、CPython 3.8 和 CPython 3.12 这些成熟的工业级 PVM 实现进行小规模补充对比，以更直观地说明现阶段 S.A.A.U.S.O VM 大致所处的性能位置，以及未来的性能优化大致应当优先关注哪些方向。
