...
2. **桥接层（src/api）**
   ...
   - 实现 `TryToForwardPendingExceptionToEembedderTryCatch`、`FinalizePendingExceptionAtApiBoundary`、`ToInternalObject`、`WrapRuntimeResult` 等关键转换；
...
在当前设计中，`Isolate` 不承诺“同一实例并发执行”安全；若确需跨线程共享同一实例，必须通过 `saauso::Locker` 串行化访问。  
工程实践上，强烈推荐“并行任务优先多 Isolate 隔离”； `Locker` 仅作为共享场景下的受控退路。
...
#### 3.6.1 对显式传入 Isolate* / 隐式使用 Current Isolate 的契约
...
### 3.6.2 对 Isolate Locker 的契约

1. 同一 `Isolate` 在跨线程共享时必须先加 `Locker`；
2. `Locker` 支持同线程重入；
3. 加锁与解锁必须发生在同一线程；
4. 持有 `Isolate::Scope` 时提前解锁属于违约并触发硬失败。
...
4. 失败处理（空 `MaybeLocal` + API 边界异常结算）

这条路径确保任何调用失败都先进入统一的 pending exception 流程：优先尝试转交给当前可接收的 `TryCatch`；若当前已经离开 Python 调用域且无可接收 `TryCatch`，则在 API 边界清理异常状态，避免旧异常污染后续调用。
...
3. `TryToForwardPendingExceptionToEembedderTryCatch` 只把异常转交给“深度匹配的最近 `TryCatch`”
4. `FinalizePendingExceptionAtApiBoundary` 只在返回空 `MaybeLocal` / `Nothing` 的 API 边界执行结算：若当前无可接收 `TryCatch` 且 Python 调用深度为 0，则清理 pending exception；若深度大于 0，则保留给解释器继续 unwind
...
其中，`Isolate::ThrowException()` 当前语义是“向当前传播域注入 pending exception，并尝试立即转交给 Embedder TryCatch”；它不参与“返回空 `MaybeLocal` / `Nothing` 的 API 边界结算”，因此不应被当作“无人接收时自动清理异常”的入口。
...
2. 失败时优先尝试把异常转交给当前可接收的 `TryCatch`
3. 若当前 API 本身已经通过空 `MaybeLocal` / `Nothing` 报告失败，则在 API 边界执行异常结算：离开 Python 调用域时清理 pending exception，仍在 Python 调用域时保留
4. 返回空 `MaybeLocal` 或 `false`
...
1. **单元测试轨**：覆盖参数校验、越界、重入、异常穿透、并发回归（含 `Locker` 负向契约）、多 Context 隔离；
...
2. 同一 `Isolate` 不保证并发执行安全；跨线程共享必须使用 `Locker` 串行化；
3. 前端编译能力依赖开关与构建配置；
...
5. 线程标识抽象：`src/execution/thread-id.h`
6. 线程锁状态基础设施：`src/execution/thread-state-infras.h`
...
4. Embedder 多线程并发能力测试：`test/embedder/embedder-thread-unittest.cc`
5. Embedder Locker 并发契约测试：`test/embedder/embedder-isolate-locker.cc`
6. 测试构建文件：`test/embedder/BUILD.gn`

### 11.6 文档入口（建议阅读顺序）

1. 接入层文档（先读）：[Saauso-Embedder-API-User-Guide.md](file:///e:/MyProject/S.A.A.U.S.O/docs/Saauso-Embedder-API-User-Guide.md)
2. 架构层文档（当前文档）：[Embedder-API-Architecture-Design.md](file:///e:/MyProject/S.A.A.U.S.O/docs/architecture/Embedder-API-Architecture-Design.md)
